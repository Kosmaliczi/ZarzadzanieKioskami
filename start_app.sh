...
# start_app.sh
# 
# Skrypt do automatycznego pobierania i wyświetlania PDF-ów i obrazów w pełnym ekranie
# PDF-y są automatycznie konwertowane na obrazy wysokiej jakości dla lepszego wypełnienia ekranu
#
# WYMAGANE OPROGRAMOWANIE:
# - curl (pobieranie plików)
# - feh, imv, eog lub display (wyświetlanie obrazów)
# - pdftoppm, ImageMagick (convert) lub Ghostscript (gs) - konwersja PDF
# - xrandr (obrót ekranu)
#
# INSTALACJA (Debian/Ubuntu):
# sudo apt-get install curl feh poppler-utils imagemagick xorg
#
# UŻYCIE:
# ./start_app.sh                          # Uruchom pętlę wyświetlania
# ./start_app.sh rotate-display right     # Obróć ekran pionowo
# ./start_app.sh rotate-display normal    # Przywróć orientację poziomą
...
open_image() {
    ...
    case "$IMG_VIEWER" in
        feh)     
            # Pełny ekran, wypełnienie bez czarnych pasków (cover mode)
            # --zoom fill wymusza wypelnienie calego ekranu
            feh -F --zoom fill "$prepared" & 
            ;;
        imv)     
            # Pełny ekran z wypełnieniem (full mode)
            imv -f -s full "$prepared" & 
            ;;
        eog)     
            # EOG nie ma opcji fill, więc dodajemy zoom do rozmiaru okna
            eog --fullscreen "$prepared" & 
            ;;
        display) 
            # ImageMagick display - rozciągnięcie na cały ekran
            # Wyswietl przygotowany obraz
...
convert_pdf_to_image() {
    ...
    local output_prefix="$IMG_DIR/${pdf_name}_page"
    local first_image=""

    shopt -s nullglob
    for f in "${output_prefix}-"*.png; do
        first_image="$f"
        break
    done
    shopt -u nullglob
...
pobierz_pdf() {
    ...
    if [ -f "$sciezka" ]; then
        ...
        http_code=$(curl -L -sS -z "$sciezka" -w "%{http_code}" -o "$tmp" "$url" || echo "000")
        if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ] && [ -s "$tmp" ]; then
            mv "$tmp" "$sciezka"
            # Wymus przebudowanie cache obrazkow po aktualizacji PDF
            rm -f "${IMG_DIR}/${base_name}_page-"*.png "$IMG_DIR/${base_name}.png"
            rm -f "/tmp/kiosk_fullscreen/${base_name}"_* 2>/dev/null || true
            echo "$(date '+%Y-%m-%d %H:%M:%S') Zaktualizowano PDF i wyczyszczono cache: $name" >> "$PDF_DIR/download.log"
        else
            rm -f "$tmp"
            if [ "$http_code" = "304" ]; then
                echo "$(date '+%Y-%m-%d %H:%M:%S') Brak zmian: $name" >> "$PDF_DIR/download.log"
...
if [ -z "$FIRST_PDF_URL" ]; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') Nie znaleziono żadnego PDF na $TARGET_PAGE - kontynuuję bez PDF" >> "$PDF_DIR/download.log"
else
    pobierz_pdf "$FIRST_PDF_URL"
fi

# Ustaw domyślną orientację ekranu na "right" (pionowo) przy starcie
echo "$(date '+%Y-%m-%d %H:%M:%S') Ustawiam orientację ekranu: right (pionowo)" >> "$PDF_DIR/download.log"
rotate_display "right" || echo "$(date '+%Y-%m-%d %H:%M:%S') UWAGA: Nie udało się obrócić ekranu" >> "$PDF_DIR/download.log"

# Przygotowanie pętli prezentacji: PDF + obrazy z katalogu
shopt -s nullglob nocaseglob

# Pętla główna wyświetlania
while true; do
    cleanup_fullscreen_cache
    ...
    if [ ${#FILTERED_MEDIA_LIST[@]} -eq 0 ]; then
        echo "$(date '+%Y-%m-%d %H:%M:%S') Brak pasujących plików dla reguły daty (szukane: ${ACTIVE_KEYS[*]})" >> "$PDF_DIR/download.log"
        sleep 5
        continue
    fi
