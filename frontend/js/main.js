 ...
 * @returns {boolean} true jeśli jest admininstrator, false w przeciwnym razie
 */
function isAdmin() {
...
/**
 * Wyświetl komunikat o braku uprawnień
 * @param {string} action - Opis akcji, która wymagana jest uprawnień
 */
function showNoPermissionError(action = '') {
    const message = action 
        ? `Brak uprawnień do wykonania tej czynności (${action}). Tylko administratorzy mogą to robić.`
        : 'Brak uprawnień do wykonania tej czynności. Tylko administratorzy mogą to robić.';
    showToast(message, 'error');
    console.warn('Próba wykonania akcji bez uprawnień:', action);
    addActivity(`Blokada: Próba wykonania akcji bez uprawnień (${action})`);
}
...
/**
 * Ograniczenie dostępu na podstawie roli użytkownika
 */
function restrictAccessByRole() {
    ...
    if (userRole !== 'admin') {
        // Ukryj sekcję ustawień w nawigacji
        ...
        // Zapobiegaj dostępowi do sekcji settings poprzez bezpośredni dostęp lub przeskakiwanie
        const settingsSection = document.getElementById('settings');
        if (settingsSection) {
            settingsSection.style.display = 'none';
        }
        
        // Ukryj przycisk dodawania kiosku
        const addKioskBtn = document.getElementById('add-kiosk-btn');
        if (addKioskBtn) {
            addKioskBtn.style.display = 'none';
        }
        
        // Ukryj formularz tworzenia nowych użytkowników
...
function initUI() {
    ...
    // Zastosuj ograniczenia dostępu na podstawie roli
    ...
    // Obsługa zarządzania użytkownikami
    setupUsers();
...
function switchSection(sectionId) {
    // Sprawdź uprawnienia dostępu do sekcji ustawień
    if (sectionId === 'settings' && localStorage.getItem('userRole') !== 'admin') {
        showToast('Brak uprawnień do dostępu do ustawień. Tylko administratorzy mogą to robić.', 'error');
        addActivity('Blokada: Próba dostępu do ustawień bez uprawnień');
        console.warn('Próba dostępu do sekcji settings bez uprawnień administratora');
        return;
    }
...
function setupForms() {
    ...
    addKioskForm.addEventListener('click', async () => {
        ...
        if (localStorage.getItem('userRole') !== 'admin') {
            showToast('Brak uprawnień do dodawania kiosków. Tylko administratorzy mogą to robić.', 'error');
            addActivity('Blokada: Próba dodania kiosku bez uprawnień');
            console.warn('Próba wykonania operacji POST Kiosk bez uprawnień administratora');
            return;
        }
    ...
    editKioskForm.addEventListener('click', async () => {
        // Sprawdź uprawnienia
        if (localStorage.getItem('userRole') !== 'admin') {
            showToast('Brak uprównień do edytowania kiosków. Tylko administratorzy mogą to robić.', 'error');
            addActivity('Blokada: Próba edycji kiosku bez uprawnień');
            console.warn('Próba wykonania operacji PUT Kiosk bez uprawnień administratora');
            return;
        }
...
function updateKiosksTableOnly() {
    ...
    kiosksData.forEach(kiosk => {
       ...
       // Przycisk SSH - tylko dla administratorów
       if (userRole === 'admin') {
           const sshBtn = document.createElement('button');
    ...
    rotateBtn.title = 'Obróć ekran (right/normal)';
       ...
       rotateBtn.addEventListener('click', async () => {
           // Sprawdź uprawnienia
           if (localStorage.getItem('userRole') !== 'admin') {
               showToast('Brak uprawnień do obracania ekranu. Tylko administratorzy mogą to robić.', 'error');
               addActivity('Blokada: Próba obrotu ekranu bez uprawnień');
               return;
           }
       ...
       // Przycisk usuwania - tylko dla administratorów
       if (userRole === 'admin') {
           const deleteBtn = document.createElement('button');
           deleteBtn.classList.add('btn', 'danger');
           deleteBtn.innerHTML = '<i class="fas fa-trash"></i>';
...
function openEditKioskModal(kiosk) {
   // Sprawdź uprawnienia
   if (!isAdmin()) {
       showNoPermissionError('edycja kiosku');
       return;
   }
...
async function deleteKiosk(kioskId, kioskIdentifier) {
   // Sprawdź uprawnienia
   if (!isAdmin()) {
       showNoPermissionError('usuwanie kiosku');
       return;
   }
...
function connectSSH(kiosk) {
   // Sprawdź uprawnienia
   if (!isAdmin()) {
       showNoPermissionError('połączenie SSH');
       return;
   }
...
function connectVNC(kiosk) {
    // Sprawdź uprawnienia
    if (!isAdmin()) {
        showNoPermissionError('połączenie VNC');
        return;
    }
...
async function restartKioskService(kiosk) {
    // Sprawdź uprawnienia
    if (!isAdmin()) {
        showNoPermissionError('restart usługi kiosku');
        return;
    }
