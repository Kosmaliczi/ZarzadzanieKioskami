...
async function handleKioskSelection(event) {
    ...
    try {
        ...
        const preferredSshPort = parseInt((typeof CONFIG !== 'undefined' && CONFIG.defaultSshPort) || 22, 10);

        // Ustaw dane połączenia; domyślnie preferujemy SFTP (port 22)
        playlistFtp.connection = {
            hostname: currentKiosk.ip_address,
            port: Number.isFinite(preferredSshPort) ? preferredSshPort : 22,
            username: (ftpCredentials && (ftpCredentials.username || ftpCredentials.ftp_username)) || ((typeof CONFIG !== 'undefined' && CONFIG.defaultSshUsername) || 'root'),
 ...
 */
async function loadPlaylistFiles(hasRetried = false) {
    ...
    try {
        ...
        const mediaPath = '/storage/videos';
        ...
        if (result && Array.isArray(result)) {
            ...
            loadScheduleFile('/storage/schedule.json');
        } else {
        ...
        const canRetryWithOtherPort = !hasRetried && (errorMessage.includes('serwerem ftp') || errorMessage.includes('10061'));

        if (canRetryWithOtherPort && playlistFtp.connection) {
            const previousPort = Number(playlistFtp.connection.port) || 21;
            const fallbackPort = previousPort === 22 ? 21 : 22;
            playlistFtp.connection.port = fallbackPort;

            showToast(`Nieudane polaczenie na porcie ${previousPort}. Proba ponowna na porcie ${fallbackPort}...`, 'warning');
            await loadPlaylistFiles(true);
            return;
        }
