...
function encryptSettingsValues(settings) {
    ...
    sensitiveKeys.forEach(key => {
        if (key in encryptedSettings && encryptedSettings[key]) {
            ...
            console.log(`Zaszyfrowano dane dla klucza ${key}`);
        }
    });
...
async function updateConfig(key, value) {
    if (key in CONFIG) {
        ...
        try {
            ...
            // Szyfruj hasła i wrażliwe dane przed zapisem w bazie
            if (key === 'defaultFtpPassword') {
                settings[key] = encryptData(value);
                console.log(`Zaszyfrowano dane dla ${key} przed zapisem`);
            } else {
                settings[key] = value;
            }
