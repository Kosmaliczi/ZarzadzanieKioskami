...
API_BASE_URL = "http://10.0.0.117:5000/api/"
...
def normalize_serial(serial):
    """
    Normalizuje numer seryjny do formatu zgodnego z wpisami w systemie.
    """
    if not serial:
        return None
...
def report_device_ip(api_base_url, device_serial, ip_address):
    ...
    """
    normalized_serial = normalize_serial(device_serial)
    if not normalized_serial:
        print("Błąd: pusty numer seryjny - pomijam raport IP.")
        return

    endpoint_url = f"{api_base_url.rstrip('/')}/device/{quote(normalized_serial, safe='')}/ip"
...
if __name__ == "__main__":
    ...
    current_serial = normalize_serial(get_device_serial())
