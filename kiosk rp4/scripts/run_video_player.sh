...
# Prefer native DRM/KMS path for true fullscreen kiosk playback on Raspberry Pi.
export SDL_VIDEODRIVER="${SDL_VIDEODRIVER:-kmsdrm}"
export SDL_RENDER_DRIVER="${SDL_RENDER_DRIVER:-KMSDRM}"
export SDL_VIDEO_MINIMIZE_ON_FOCUS_LOSS=0

# If no explicit desktop backend is requested, detach from X/Wayland session
# so startup overlays (console/RPi Connect windows) don't sit above playback.
if [[ -z "${KIOSK_KEEP_DESKTOP_SESSION:-}" ]]; then
    unset DISPLAY || true
