...
bool AudioManager::initialize(AVCodecContext* codecContext, AVStream* stream) {
    if (!codecContext || !stream) {
        Logger::logError("Audio initialization failed: null codec context or stream");
        return false;
    }

    if (codecContext->sample_rate <= 0 || codecContext->ch_layout.nb_channels <= 0) {
        Logger::logError(
            "Audio initialization failed: invalid stream parameters (sample_rate=" +
            std::to_string(codecContext->sample_rate) + ", channels=" +
            std::to_string(codecContext->ch_layout.nb_channels) + ")"
        );
        return false;
    }
